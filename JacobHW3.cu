...
void setUpDevices()
{
	...
	GridSize.x = (N-1) / BlockSize.x + 1; // changed the gridsize to the formula we derived in class. 
	GridSize.y = 1;
	GridSize.z = 1;
}
...
{
	int id = blockIdx.x * blockDim.x + threadIdx.x; // changed to fit the correct integer using this formula from class.
	
	if(id < n)                                      // changed to an if statement.
	{
		c[id] = a[id] + b[id];                      // deleted a line of code below this one.
	}
}
