...
int PaddedN; 
cudaDeviceProp deviceProp; // This variable will hold the properties of your GPU
...
void chooseGPU()
{
	int deviceCount, bestDevice = 0;
	cudaDeviceProp prop;
	double bestCC = -1.0, cc;
	...
	if(deviceCount == 0)
	{
		printf("\n No CUDA-capable GPU was found. Exiting.\n");
		exit(0);
	}
	...
	{
		printf("\n Selected GPU (%s, compute capability %d.%d) does not support", deviceProp.name, deviceProp.major, deviceProp.minor);
		printf(" atomic add on floats (needs 3.0+). Exiting.\n");
		exit(0);
	}
}
...
void setUpDevices()
{
	...
	{
		printf("\n BLOCK_SIZE (%d) is not a power of 2. Exiting.\n", BLOCK_SIZE);
		exit(0);
	}
	...
	GridSize.y = 1;
	GridSize.z = 1;
 	if(BlockSize.x > (unsigned int)deviceProp.maxThreadsPerBlock)
	{
		printf("\n BLOCK_SIZE (%d) exceeds this GPU's maxThreadsPerBlock (%d). Exiting.\n", BLOCK_SIZE, deviceProp.maxThreadsPerBlock);
		exit(0);
	}
	if(GridSize.x > (unsigned int)deviceProp.maxGridSize[0])
	{
		printf("\n Grid size (%d) exceeds this GPU's maxGridSize[0] (%d). Exiting.\n", GridSize.x, deviceProp.maxGridSize[0]);
		exit(0);
	}

	PaddedN = GridSize.x*BlockSize.x; // N rounded up to a multiple of BLOCK_SIZE.
}
...
{	
	...
	// Explicitly zero the device vectors so the padding elements are 0,
	// instead of hoping leftover GPU memory happens to be zero.
...
int main()
{
	...
	// Copy the final summed result back from GPU to CPU.
	// (No CPU-side loop needed anymore -- atomicAdd already did the reduction.)
	cudaMemcpyAsync(&DotGPU, C_GPU, sizeof(float), cudaMemcpyDeviceToHost);
	cudaErrorCheck(__FILE__, __LINE__);
