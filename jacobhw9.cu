...
int PaddedN; // This variable will hold the length of the vector after padding it with zeros.
...
void setUpDevices()
{
	...
	PaddedN = GridSize.x * BlockSize.x; // This is the length of the vector after padding it with zeros.
}
...
{	
	...
	C_CPU = (float*)malloc(PaddedN*sizeof(float));
...
{
	extern __shared__ float sharedC[];
	int id = threadIdx.x + blockIdx.x * blockDim.x;
	
	sharedC[threadIdx.x] = a[id] * b[id];
	__syncthreads();
