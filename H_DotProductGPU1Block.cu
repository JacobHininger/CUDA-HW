...
/*
 ...
 1. Changed the size of the memory copy from GPU to CPU to use BlockSize.x instead of N.
 2. Added dotProductGPU kernel
 3. Added __synchthreads() to the dotProductGPU kernel to ensure all threads have completed their work before proceeding with the reduction step.
*/
...
{	
	...
	C_CPU = (float*)malloc(BlockSize.x*sizeof(float)); // changed N to blocksize.x
	...
	cudaErrorCheck(__FILE__, __LINE__);
	cudaMalloc(&C_GPU,BlockSize.x*sizeof(float)); // changed N to blocksize.x
	cudaErrorCheck(__FILE__, __LINE__);
}
...
__global__ void dotProductGPU(float *a, float *b, float *C_GPU, int n)
{
	...
	{
		C_GPU[id] = 0.0; // if the thread id is greater than the size of the vector, set the value to 0.0
	}
	__syncthreads();
	...
	while(fold > 1)
	{
		if(fold % 2 != 0) // if the fold is odd, add the last element to the first element
		{
			if(id == 0)
			{
				C_GPU[0] += C_GPU[fold - 1]; // if the fold is odd, add the last element to the first element
			}
			fold = fold - 1;
		}
		fold = fold/2;	
		
		if(id < fold) // if the thread id is less than the fold, add the two elements together
		{
...
int main()
{
	...
	cudaMemcpyAsync(C_CPU, C_GPU, BlockSize.x*sizeof(float), cudaMemcpyDeviceToHost);
	cudaErrorCheck(__FILE__, __LINE__);
